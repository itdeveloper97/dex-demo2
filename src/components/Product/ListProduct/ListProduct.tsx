...
import {changeOrderProductActionCreator, deleteProductActionCreator, useProductSelector} from "../../../store/product";
...
export const ListProduct = () => {
  ...
  const order = useProductSelector(state => state.product.order)
  const items = useProductSelector(state => state.product.items)


  const onGoAddProduct = () => history.push('/add-product')
  const onGoEditProduct = (productID: string) => history.push(`/edit-product/${productID}`)
  const onGoToProduct = (productID: string) => history.push(`/product/${productID}`)
  const onChangeOrder = () => dispatch(changeOrderProductActionCreator())
  const onDeleteItem = (productID: string) => {
    dispatch(deleteProductActionCreator({id: productID}))
...
const List: React.FC<{
  ...
  onGoToProduct: (itemID: string) => void
}> = (
  {
    items,
    order,
  ...
  const [showItems, setShowItems] = useState<ProductInterface[] | null>(null)
      ...
      <Body>
...

        {
          showItems && showItems.slice(limitFromBy[0], limitFromBy[1]).map((item, indexItem) => (
            <TR key={indexItem}>
              <TD onClick={() => onGoToProduct(item.id)} control>
                {item.name}
              </TD>
              <TD>
                {item.price}
              ...
              <TD control>
                <div onClick={() => onEditItem(item.id)}>
                  Ред.
                </div>
